...
// Thời gian chờ để tránh tắc nghẽn (ms)
...
void cb_ap_CaptiveTasks(void){
    handleCaptiveTasks();   // ← Gọi liên tục để captive portal hoạt động
}
...
void UART_Init()
{
    digitalWrite(W5500_CS, HIGH); // Tắt W5500 để tránh xung đột SPI
    digitalWrite(SCREEN_CS, LOW); // Kích hoạt màn hình
    UART_Initialization();
...
void Main_Loop()
{
    ...
    unsigned long lastTouchRead = 0; // Thời gian đọc cảm ứng lần cuối

    // Khởi tạo cả hai hệ thống
    Ethernet_Init();
    UART_Init();
    // Chỉ hiển thị giao diện Ethernet
    Ethernet_Display();
    ...
    while (1)
    {
        if (Skip_Current_Test == true)
        {
            digitalWrite(W5500_CS, HIGH);
            digitalWrite(SCREEN_CS, HIGH);
            break; // Thoát nếu Skip_Current_Test được kích hoạt
        }

        // Cập nhật cả Ethernet và UART ở chế độ nền
        GFX_Print_Ethernet_Info_Loop();
        GFX_Print_UART_Info_Loop();
        Handle_Read_Beam();

        // Đọc cảm ứng với tần suất giới hạn
        if (millis() - lastTouchRead >= TOUCH_READ_INTERVAL)
        {
            ...
            if (fingers_number > 0)
            {
                ...
                // Chỉ xử lý nút Try connect (Retry) cho Ethernet
        ...
        delay(LOOP_DELAY); // Tránh CPU chạy 100%
    }
}

/* void printer_send_data(const uint8_t *data, size_t len) {
    Serial2.write(data, len);  // Serial2 là cổng UART nối với máy in
}
 */
...
void setup()
{
    ...
    soft_timer_init();
    soft_timer_start(1, true, cb_tcp_check_connection); // Kiểm tra kết nối mỗi 1 giây
    ...
    //soft_timer_start(10000, true, cb_printer);
    soft_timer_start(100, true, cb_process_even);
#if ENABLE_WIFI_AP
    soft_timer_start(500, true, cb_ap_CaptiveTasks); //100–200 ms	nếu muốn phản ứng nhanh hơn
#endif
