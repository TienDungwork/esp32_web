...
void printer_set_bold(bool enable) {
    const uint8_t cmd[] = { 0x1B, 0x45, enable ? 1 : 0 };
    send_command(cmd, sizeof(cmd));
}

void printer_set_underline(bool enable) {
    const uint8_t cmd[] = { 0x1B, 0x2E, enable ? 1 : 0 };
    send_command(cmd, sizeof(cmd));
}

void printer_set_reverse(bool enable) {
    const uint8_t cmd[] = { 0x1D, 0x42, enable ? 1 : 0 };
    send_command(cmd, sizeof(cmd));
}
...
void printer_print_barcode(const char *data, uint8_t type) {
    ...
    send_byte(0x00); // null terminated
}
...
void printer_print_bitmap(const uint8_t *bitmap_data, uint16_t width, uint16_t height) {
    ...
    for (uint16_t row = 0; row < height; row += 8) {
        // Chuẩn bị lệnh ESC * m nL nH
        uint8_t nL = bytes_per_row & 0xFF;
        uint8_t nH = (bytes_per_row >> 8) & 0xFF;
        uint8_t cmd[] = {0x1B, 0x2A, 0x00, nL, nH};  // m = 0 -> 8-dot single-density

        printer_send_data(cmd, sizeof(cmd));

        // Gửi dữ liệu từng cột
...
// In tiếng Việt (demo)
// =========================
char convert_utf8_to_ascii(uint8_t c) {
    // Ánh xạ đơn giản cho ví dụ demo. Cần mở rộng nếu dùng Unicode đầy đủ.
    switch (c) {
        case 0xE1: return 'a';  // á
        case 0xBA: return 'o';  // ơ
        case 0xE0: return 'a';  // à
        // ...
        default: return c;
    }
}
...
void printer_print_vietnamese(const char *utf8_text) {
    while (*utf8_text) {
        ...
        if (c >= 0xC0) {
            // ký tự unicode 2 byte, ánh xạ đơn giản
