...
JSMN_API int json_parse_uint64(const char *json, jsmntok_t *tok, uint64_t* value)
{
	char data[11];
	int j, k = 0;
...
int json_parse_uint(const char *json, jsmntok_t *tok, int *value) {
    char data[11];  // đủ cho số nguyên 32-bit
    int j, k = 0;
    for (j = tok->start; j < tok->end && k < sizeof(data) - 1; ++j, ++k)
        data[k] = json[j];
    data[k] = '\0';
